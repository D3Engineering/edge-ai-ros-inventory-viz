...
class datamatrix_visualizer:
    """
    datamatrix_visualizer was a standalone test of DataMatrix detection on the robot
    It expects an image to be available at /left/imx390/image_raw_rgb, from which it detects DataMatrix codes,
    applies markup (detection boxes, and labels of what data is encoded), and republishes at /left/imx390/image_fused
    """
    ...
    def image_callback(self, image):
        """This function is run by the subscriber when an image is received for processing

        It runs the decode() function from pylibdmtx to gather detection info, then applies markup
        to each detection using OpenCV, publishing the marked up image

        :argument image ROS Image Message, passed in by a Subscriber
        """
        ...
        try:
            ...
            for d in detections:
                top = 1096 - d.rect.top
                left = d.rect.left
                self.current_image = cv2.rectangle(self.current_image, (left, top),
                                                   (left + d.rect.width, top - d.rect.height), (255, 0, 0), 2)
                self.current_image = cv2.putText(self.current_image, str(d.data), (left, top - d.rect.height),
